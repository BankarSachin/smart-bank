package com.smartbank.accountservice.config;
...
public class GlobalFeignClientErrorDecoder implements ErrorDecoder {
    ...
    public Exception decode(String methodKey, Response response) {
    	if (response.body()==null) {
			...
			return new AccsException(ExceptionCode.ACCS_RMTE_SERVICE_EMPTY_RESPONSE); 
		}
     ...
     */
    private AccsException extractException(Response response) {
    	final String methodName = "extractException";
    	AccsException globalException = null;
        ...
        try(Reader reader = response.body().asReader(StandardCharsets.UTF_8)) {
			...
			globalException = new AccsException(ExceptionCode.ACCS_RMTE_SERVICE_EXCEPTION, errorInfo.getCode(),errorInfo.getCauses().get(0));
        } catch (IOException e) {
